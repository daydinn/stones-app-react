  ...
  useEffect(() => {
    if (user) {
      const fetchCrystals = async () => {
        const q = query(collection(db, 'crystals'), where('userId', '==', user.uid));
        const querySnapshot = await getDocs(q);
        const crystalsArray = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-6">
      ...
      {crystals.map(crystal => (
        <div key={crystal.id} className="p-4 mb-4 bg-gray-100 rounded-lg shadow-md">
          ...
          <p><strong>Zodiac Signs:</strong> {crystal.zodiacs.join(', ')}</p>
          ...
          <div className="flex flex-wrap mt-2">
            {crystal.images.map((image, index) => (
              <div key={index} className="relative w-24 h-24 m-1">
                <img src={image} alt={`crystal-${index}`} className="w-full h-full object-cover rounded-md" />
              </div>
            ))}
          </div>
        </div>
