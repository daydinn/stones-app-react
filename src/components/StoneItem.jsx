  ...
  return (
    <li className="relative bg-white flex flex-col justify-between  items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]">
      <Link className="contents" to={`/${stone.name}/${id}`}>
        ...
        />
        <div
          className="absolute top-2 left-2 bg-fuchsia-500 text-white uppercase text-xs font-medium rounded-md px-2 py-1 shadow-lg">
          {stone.favorite ? <MdFavorite /> : <MdFavoriteBorder />}
          
        </div>
        <div className="w-full p-[10px]">
          <div className="flex">
            <p className="font-bold m-0 text-xl truncate mr-2  ">{stone.name}</p>
            <div className="flex items-center mt-1">
          {stone.elements.map((element, index) => (
                <span key={index} className="text-sm rounded-full  border border-gray-300 ">
